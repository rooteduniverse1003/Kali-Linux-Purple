...
IMAGE_NAME=kali-malcolm
IMAGE_PUBLISHER=kali
IMAGE_VERSION=0.0.2
ISO_VERSION=05
IMAGE_DISTRIBUTION=kali-rolling
...
if [ -d "$WORKDIR" ]; then
  ...
  [[ -n $YML_IMAGE_VERSION ]] && IMAGE_VERSION="$YML_IMAGE_VERSION-$ISO_VERSION"
  ##sed -i "s@^\(title-text[[:space:]]*:\).*@\1 \"Malcolm Kali Rolling 2023.1\"@g" ./config/bootloaders/grub-pc/live-theme/theme.txt
  ...
  echo "VARIANT=\"Hedgehog Kali Linux (Malcolm) v${IMAGE_VERSION}\""              >> "$MALCOLM_DEST_DIR"/.os-info
  echo "VARIANT_ID=\"kali-hedgehog-malcolm\""                                     >> "$MALCOLM_DEST_DIR"/.os-info
  ...
  echo "SUPPORT_URL=\"https://forums.kali.org/\""                                 >> "$MALCOLM_DEST_DIR"/.os-info
  echo "BUG_REPORT_URL=\"https://github.com/kalilinux/build-scripts/kali-malcolm/issues\""  >> "$MALCOLM_DEST_DIR"/.os-info
  ...
  lb config \
    ...
    --debootstrap-options "--include=apt-transport-https,bc,ca-certificates,gnupg,jq,openssl --no-merged-usr --keyring=/usr/share/keyrings/kali-archive-keyring.gpg" \
    --debootstrap-script /usr/share/debootstrap/scripts/kali-rolling \
    --distribution $IMAGE_DISTRIBUTION \
    --distribution-binary $IMAGE_DISTRIBUTION \
    ...
    --parent-debian-installer-distribution "kali-rolling" \
    --security false \
    --source false \
    --updates false \
