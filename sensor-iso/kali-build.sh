...
if [ -d "$WORKDIR" ]; then
  ...
  sed -i "s@^\(title-text[[:space:]]*:\).*@\1 \"Hedgehog Kali Linux $IMAGE_VERSION $(date +'%Y-%m-%d %H:%M:%S')\"@g" ./config/bootloaders/grub-pc/live-theme/theme.txt
  ...
  if [ -f "$SCRIPT_PATH/interface/requirements.txt" ]; then
    ...
    echo -n "pip3 install --break-system-packages --no-compile --no-cache-dir --force-reinstall --upgrade" >> ./config/hooks/normal/0168-pip-sensor-interface-installs.hook.chroot
    while read LINE; do
  ...
  echo "VARIANT=\"Hedgehog Kali Linux (Sensor) v${IMAGE_VERSION}\""                    >> ./config/includes.chroot/opt/sensor/.os-info
